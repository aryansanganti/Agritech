        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 backdrop-blur-xl animate-fade-in">
            ...
            <div className="flex flex-col items-center gap-12 w-full max-w-lg px-6">
                <div className="text-center space-y-2">
                    <h2 className="text-3xl font-heading font-bold text-bhumi-primary dark:text-bhumi-darkPrimary">
                        Bhumi Live
                    </h2>
                    <p className="text-bhumi-mutedFg dark:text-bhumi-darkMutedFg text-lg min-h-[60px]">{transcript}</p>
                </div>
                ...
                <button 
                    ...
                    `}
                >
                    <div className="absolute inset-0 border-4 border-white/20 blur-sm"></div>
                    {status === 'idle' && <Mic size={48} className="text-white/80" />}
                    {status === 'listening' && <div className="w-20 h-20 bg-white/20 animate-ping"></div>}
                    {status === 'thinking' && <Sparkles size={48} className="text-[#E8D4A5] animate-pulse" />}
