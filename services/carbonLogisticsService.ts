...
/**
 * Calculate simple distance between two districts (approximate)
 */
export const calculateRouteDistance = (district1: string, district2: string): number => {
    // Mock logic - strictly for demo visualization if coordinates aren't available
    const d1 = district1.length;
    const d2 = district2.length;
    return Math.abs(d1 - d2) * 10 + 50;
};

/**
 * Calculate carbon footprint based on distance and mode
 */
export const calculateCarbonFootprint = (distance: number, mode: 'truck' | 'train' | 'flight' = 'truck'): number => {
    const factors = {
        truck: 0.15, // kg CO2 per km
        train: 0.05,
        flight: 0.50
    };
    return Math.round(distance * factors[mode] * 100) / 100;
};
